...
?>
<style>.colorImg{border-radius:50%;overflow:hidden}.colorImg img{width:30px;height:30px}.colorImg img:hover{transform:scale(1.1);transition:1s}.shopnow{background-color:#e1d106;color:#000;text-decoration:none;padding:10px;margin:auto;text-align:center;font-size:1rem;border:0;border-radius:4px;width:200px}.shopnow:hover{background-color:#000;color:#e1d106}.fa{color:red;font-size:30px}
...
<style>#divgrandImg{margin:0;width:480px;height:618px;position:relative;cursor: none;}#loupe{width:200px;height:200px;border-radius:50%;border:3px solid #fff;position:absolute;background-size:cover;background-repeat:no-repeat;box-shadow:0 0 10px rgba(0,0,0,0.6);filter:brightness(1.1);display:none;z-index:10}</style>
<?php 
$titreProd=$row['titre'];
echo "<script>document.title = '$titreProd'; </script>";
?>
<body>
<div id="page">
...
<div class="col-sm">
<div class="row">
<div class="col-3" style="display:grid;justify-content:center;align-items:center;align-content: space-around;">
...
<p>Coleur</p>
<?php 
$categorie=$row['categorie'];$sql1="SELECT * FROM `produits` WHERE `categorie`='$categorie' ORDER BY `date` DESC LIMIT 3";$result1=$cnx->query($sql1);while($row1=$result1->fetch_assoc()){$nvprix=$row['prix']-$row['prix']*($row['remis']/100); ?>
...
<?php 
$taille=explode("-",$row['taille']);for($x=0;$x<=count($taille)-2;$x++){echo' '.$taille[$x].' ';}?>
</p>
</div>
</div>
<div>
...
<?php $reff=$row['Ref'];$sqlF="SELECT * FROM `favor` WHERE `RefProd` ='$reff'";$resultF=$cnx->query($sqlF);if($rowF=$resultF->fetch_assoc()){?>
<a class="lienfav p-1" href="deletfavor.php?ref=<?php echo $row['Ref'] ;?>&refclient=<?php echo  $_SESSION["Reff"] ;?>">
<i class="fa fa-heart" aria-hidden="true"></i></a>
<?php }else{ ?>
<a class="lienfav p-1" href="ajouterfavor.php?ref=<?php echo $row['Ref'] ;?>&refclient=<?php echo  $_SESSION["Reff"] ;?>">
...
<script>function change1(){document.getElementById("grandImg").src=document.getElementById("img1").src;document.getElementById("loupe").style.backgroundImage="url('"+document.getElementById("img1").src+"')"}function change2(){document.getElementById("grandImg").src=document.getElementById("img2").src;document.getElementById("loupe").style.backgroundImage="url('"+document.getElementById("img2").src+"')"}function change3(){document.getElementById("grandImg").src=document.getElementById("img3").src;document.getElementById("loupe").style.backgroundImage="url('"+document.getElementById("img3").src+"')"}function change4(){document.getElementById("grandImg").src=document.getElementById("img4").src;document.getElementById("loupe").style.backgroundImage="url('"+document.getElementById("img4").src+"')"}function change5(){document.getElementById("grandImg").src=document.getElementById("img5").src;document.getElementById("loupe").style.backgroundImage="url('"+document.getElementById("img5").src+"')"}</script>
<script>zoom=1.5;document.getElementById("divgrandImg").onmousemove=function(){loupe=document.getElementById("loupe");var a=document.getElementById("divgrandImg").getBoundingClientRect();loupe.style.left=(event.clientX-a.left)+"px";loupe.style.top=(event.clientY-a.top)+"px";loupe.style.backgroundSize=(498*zoom)+"px";loupe.style.backgroundPosition=(-loupe.offsetLeft*zoom)+"px "+(-loupe.offsetTop*zoom)+"px"};function afficherloupe(){document.getElementById("loupe").style.display="block"}function fermerloupe(){document.getElementById("loupe").style.display="none"};</script>
</body>