<?php 
$c=$_GET['c'];$p=$_GET['p'];$t=$_GET['t'];
?>
<?php include 'header.php';?>
<body>
<style>#pro2,#pro3{display:none}.carousel-indicators li{background-color:#EDCA51!important}#carouselExampleIndicators{height:500px;overflow:hidden}.imgGrande{overflow:hidden;display:flex;justify-content:center;align-items:center}.colorImg{border-radius:50%;overflow:hidden}.colorImg img{width:30px;height:30px}.colorImg img:hover{transform:scale(1.1);transition:1s}.favor .fa{color:red}.btnDetail{width:auto;height:40px;color:#000;display:flex;justify-content:center;align-items:center;background-color:#e1d106;text-align:center;font-size:1rem}.btnDetail:hover{text-decoration:none;background-color:#000;color:#e1d106}.btnBoutique{border:0;background:transparent}.btnBoutique:hover{border:0;background:transparent}
...
<?php 
$sqlc="SELECT * FROM `produits` ORDER BY `date` DESC LIMIT 1";$resultc=$cnx->query($sqlc);while($rowc=$resultc->fetch_assoc()){ ?>
...
<?php $Ref=$rowc['Ref'];$sql1="SELECT * FROM `produits` p  WHERE  LOCATE('".substr($Ref,0,5)."', p.Ref ) AND `Ref`<>'".$Ref."'   ORDER BY `date` DESC LIMIT 2";$result1=$cnx->query($sql1);while($row1=$result1->fetch_assoc()){$nvprix=$row['prix']-$row['prix']*($row['remis']/100); ?>
...
<?php } ?>
<?php $categorie=$rowc['categorie'];$sql1="SELECT * FROM `produits` WHERE `categorie`='$categorie' ORDER BY `date` DESC LIMIT 2";$result1=$cnx->query($sql1);while($row1=$result1->fetch_assoc()){$nvprix=$row['prix']-$row['prix']*($row['remis']/100);  ?>
...
<?php $categorie=$rowc['categorie'];$sql1="SELECT * FROM `produits` WHERE `categorie`='$categorie' ORDER BY `date` DESC LIMIT 3";$result1=$cnx->query($sql1);while($row1=$result1->fetch_assoc()){$nvprix=$row['prix']-$row['prix']*($row['remis']/100); ?>
<a class="colorImg" href="product-details.php?id=<?php echo $row1['id']; ?>"><img src="<?php echo 'data:image/jpeg;base64,' . base64_encode($row1['img1']); ?>" alt=""></a>
<?php } ?>
</div>
</div>
...
<script>function cat(a){if(a=="1"){document.getElementById("cat1").classList.remove("cat");document.getElementById("cat1").classList.add("activecat");document.getElementById("cat2").classList.remove("activecat");document.getElementById("cat3").classList.remove("activecat");document.getElementById("cat4").classList.remove("activecat");document.getElementById("cat2").classList.add("cat");document.getElementById("cat3").classList.add("cat");document.getElementById("cat4").classList.add("cat");document.getElementById("group1").style.display="flex";document.getElementById("group2").style.display="none";document.getElementById("group3").style.display="none";document.getElementById("group4").style.display="none"}if(a=="2"){document.getElementById("cat2").classList.remove("cat");document.getElementById("cat2").classList.add("activecat");document.getElementById("cat1").classList.remove("activecat");document.getElementById("cat3").classList.remove("activecat");document.getElementById("cat4").classList.remove("activecat");document.getElementById("cat1").classList.add("cat");document.getElementById("cat3").classList.add("cat");document.getElementById("cat4").classList.add("cat");document.getElementById("group1").style.display="none";document.getElementById("group2").style.display="flex";document.getElementById("group3").style.display="none";document.getElementById("group4").style.display="none"}if(a=="3"){document.getElementById("cat3").classList.remove("cat");document.getElementById("cat3").classList.add("activecat");document.getElementById("cat1").classList.remove("activecat");document.getElementById("cat2").classList.remove("activecat");document.getElementById("cat4").classList.remove("activecat");document.getElementById("cat1").classList.add("cat");document.getElementById("cat2").classList.add("cat");document.getElementById("cat4").classList.add("cat");document.getElementById("group1").style.display="none";document.getElementById("group2").style.display="none";document.getElementById("group3").style.display="flex";document.getElementById("group4").style.display="none"}if(a=="4"){document.getElementById("cat4").classList.remove("cat");document.getElementById("cat4").classList.add("activecat");document.getElementById("cat1").classList.remove("activecat");document.getElementById("cat2").classList.remove("activecat");document.getElementById("cat3").classList.remove("activecat");document.getElementById("cat1").classList.add("cat");document.getElementById("cat2").classList.add("cat");document.getElementById("cat3").classList.add("cat");document.getElementById("group1").style.display="none";document.getElementById("group2").style.display="none";document.getElementById("group3").style.display="none";document.getElementById("group4").style.display="flex"}}</script>
...
<style>.parent1{display:grid;grid-template-columns:repeat(5,1fr);grid-template-rows:repeat(2,1fr);grid-column-gap:0;grid-row-gap:0;height:500px;background-color:#fff}.parent1 img{width:100%;height:100%}.parent1 div{overflow:hidden}.divp1{grid-area:1 / 1 / 3 / 2;border:1px solid #fdfdfd;background-color:#fff}.divp2{grid-area:1 / 2 / 2 / 3;border:1px solid #fdfdfd;background-color:#fff}.divp3{grid-area:2 / 2 / 3 / 3;border:1px solid #fdfdfd;background-color:#fff}.divp4{grid-area:2 / 3 / 3 / 4;border:1px solid #fdfdfd;background-color:#fff}.divp5{grid-area:1 / 4 / 2 / 5;border:1px solid #fdfdfd;background-color:#fff}.divp6{grid-area:2 / 4 / 3 / 5;border:1px solid #fdfdfd;background-color:#fff}.divp7{grid-area:1 / 5 / 3 / 6;border:1px solid #fdfdfd;background-color:#fff}.divp8{grid-area:1 / 3 / 2 / 4;background-color:#fcea01;position:relative;transform:scale(1.2);display:grid;padding:10px;border-radius:50%;align-items:flex-end;border:1px solid #fcea01}</style>
<div class="parent1">
...
<div class="col">
<div>
<button onclick="ville('<?php echo $row['ville'];?>','<?php echo $row['adresse'];?>','<?php echo $row['tele'];?>','<?php echo $row['localisation'];?>','image<?php echo $cpt;?>')" type="button" class="btnBoutique" data-toggle="modal" data-target="#exampleModal1">
...
<script>function ville(e,c,b,d,a){document.getElementById("villeboutique").innerHTML=e;document.getElementById("adresse").innerHTML=c;document.getElementById("tele").innerHTML=b;document.getElementById("lien").href=d;document.getElementById("imageville2").src=document.getElementById(a).src}</script>
</div>
</div>
